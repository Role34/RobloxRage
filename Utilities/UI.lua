...
local MainAssetFolder = Debug and ReplicatedStorage.Bloxet
    or InsertService:LoadLocalAsset("rbxassetid://10803229657")
...
local function GetConfigs()
    if not isfolder("IMissTheRage") then makefolder("IMissTheRage") end
    if not isfolder("IMissTheRage\\Configs") then makefolder("IMissTheRage\\Configs") end
    if not isfile("IMissTheRage\\DefaultConfig.txt") then writefile("IMissTheRage\\DefaultConfig.txt","") end

    local Configs = {}
    for Index,File in pairs(listfiles("IMissTheRage\\Configs") or {}) do
        File = File:gsub("IMissTheRage\\Configs\\","")
        File = File:gsub(".json","")
        Configs[Index] = File
    end
    return Configs
...
local function ConfigsToList()
    ...
    for Index,File in pairs(listfiles("IMissTheRage\\Configs") or {}) do
        File = File:gsub("IMissTheRage\\Configs\\","")
        File = File:gsub(".json","")
        local DefaultConfig = readfile("IMissTheRage\\DefaultConfig.txt")
...
local function InitScreen()
    ...
    --[[if Debug then
        ...
        RobloxRage.Utilities.Misc:HideObject(ScreenAsset)
    end]]
...
local function InitWindow(ScreenAsset,Window)
    ...
    function Window:SaveConfig(Name)
        ...
        if table.find(GetConfigs(),Name) then
            Config = HttpService:JSONDecode(readfile("IMissTheRage\\Configs\\"..Name..".json"))
        end
        for Index,Element in pairs(Window.Elements) do
        ...
        writefile("IMissTheRage\\Configs\\"..Name..".json",HttpService:JSONEncode(Config))
    end
    function Window:LoadConfig(Name)
        if table.find(GetConfigs(),Name) then
            local DecodedJSON = HttpService:JSONDecode(readfile("IMissTheRage\\Configs\\"..Name..".json"))
            for Index,Element in pairs(Window.Elements) do
    ...
    function Window:DeleteConfig(Name)
        if table.find(GetConfigs(),Name) then
            delfile("IMissTheRage\\Configs\\"..Name..".json")
        end
    end
            ...
            local ConfigSection = Tab:Section({Name = "Configs",Side = Side}) do
                ...
                ConfigSection:Button({Name = "Set",Callback = function()
                    if ConfigDropdown.Value and ConfigDropdown.Value[1] then
                        ...
                        writefile("IMissTheRage\\DefaultConfig.txt",DefaultConfig)
                        ConfigDivider:SetText(
                        "Default Config\n<font color=\"rgb(189,189,189)\">[ "..DefaultConfig.." ]</font>")
                    end
                end})
                ConfigSection:Button({Name = "Clear",Callback = function()
                    writefile("IMissTheRage\\DefaultConfig.txt","")
